<template>
  ...
  >
    <div class="max-w-2xl mx-auto px-4 text-center space-y-10">
      <div>
        <h2 class="text-4xl font-bold mb-5 font-heading">
          {{ gift?.title }}
        </h2>
      ...
      <div class="space-y-5">
        ...
        >
          <div class="absolute -top-10 -right-10 w-32 h-32 bg-rose-200 rounded-full" />

          <div class="relative z-10 text-left space-y-1">
            <p class="text-lg font-semibold text-red-900">
              {{ bank.name }}
            </p>
            <p class="text-gray-800 tracking-wider text-base font-mono">
              {{ bank.account }}
            </p>
